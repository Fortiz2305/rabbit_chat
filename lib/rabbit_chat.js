...
var rabbit_chat = {
  ...
  help: function () {
    const banner = fs.readFileSync(path.resolve(__dirname, '../banner.txt'));
...
Usage: rabbit_chat -option [parameter] \n\n\
  ...
  addChannel: function (user, ch_name) {
    const channelsDir = path.resolve(__dirname, '..', 'users_channels');
    try {
      fs.statSync(channelsDir);
    } catch(e) {
      fs.mkdirSync(channelsDir);
    }
    fs.appendFile(path.join(channelsDir, `${user}.txt`),
                  ch_name + '\n', 'utf8', function (err) {
                    if (err) throw err;
                  });
  },
  getChannels: function (user) {
    fs.readFile(path.resolve(__dirname, '..', 'users_channels', `${user}.txt`),
                function (err, data) {
                  if (err) throw err;
